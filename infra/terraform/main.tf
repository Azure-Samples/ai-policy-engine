...
# ---------------------------------------------------------------------------
# SPA redirect URI for the AI Policy UI on the Container App
...
# ---------------------------------------------------------------------------
# APIM managed identity → AIPolicy.Apim app role on the API app registration
# This allows the APIM policy to call the AI Policy API's precheck/log endpoints.
# ---------------------------------------------------------------------------

resource "azuread_app_role_assignment" "apim_aipolicy_role" {
...
resource "local_file" "ui_env" {
  filename = "${path.module}/../../src/aipolicyengine-ui/.env.production.local"
  content  = <<-EOT
...
DemoClient__AIPolicyBase=https://${module.compute.container_app_fqdn}
DemoClient__Clients__0__Name="AI Policy Sample Client"
...
DemoClient__Clients__1__Name="AI Policy Demo Client 2"
